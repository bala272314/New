...
public class Util {
	 ...
	 */
	public static String getCurrentFormattedTime(String strDateFormatString) {
		DateFormat dateFormat = new SimpleDateFormat(strDateFormatString);
		Calendar calendar = Calendar.getInstance();
	 ...
	 * @see #getCurrentFormattedTime(String)
	 */
	public static String getFormattedTime(Date time, String strDateFormatString) {
		DateFormat dateFormat = new SimpleDateFormat(strDateFormatString);
	...
	public static String getTimeDifference(Date startTime, Date endTime) {
		long lngTimeDifferenceSeconds = (endTime.getTime() - startTime.getTime()) / 1000; // to
																						// convert
																						// from
																						// milliseconds
																						// to
																						// seconds
		long lngTimeDifferenceMinutes = lngTimeDifferenceSeconds / 60;

		String strTimeDifferenceDetailed;
		if (lngTimeDifferenceMinutes >= 60) {
			long timeDifferenceHours = lngTimeDifferenceMinutes / 60;

			strTimeDifferenceDetailed = Long.toString(timeDifferenceHours) + " hour(s), "
					+ Long.toString(lngTimeDifferenceMinutes % 60) + " minute(s), "
					+ Long.toString(lngTimeDifferenceSeconds % 60) + " second(s)";
		} else {
			strTimeDifferenceDetailed = Long.toString(lngTimeDifferenceMinutes) + " minute(s), "
					+ Long.toString(lngTimeDifferenceSeconds % 60) + " second(s)";
		}

		return strTimeDifferenceDetailed;
	}
}